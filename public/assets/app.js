...
async function loadFiles(prefix = '') {
  ...
  try {
    ...
    state.files   = (data.objects  || []).filter(o => !o.key.endsWith('.keep') && o.key !== prefix);
...
async function uploadFiles(files) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      itemEl.querySelector('.upload-item-status').className = 'upload-item-status done';
    } catch (err) {
      itemEl.querySelector('.upload-item-status').textContent = '✗ Failed';
      itemEl.querySelector('.upload-item-status').className = 'upload-item-status error';
...
function setupContextMenu() {
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { hideCtxMenu(); closeLightbox(); }
  });
}
...
function setupModals() {
  ...
  $('mkdir-confirm').addEventListener('click', async () => {
    ...
    if (!name) return;
    const path = state.currentPath ? `${state.currentPath.replace(/\/$/, '')}/${name}` : name;
  ...
  // Rename
  $('rename-cancel').addEventListener('click', () => closeModal('modal-rename'));
  ...
  $('move-confirm').addEventListener('click', async () => {
    ...
    let destFolder = selected ? selected.dataset.prefix : customPath.replace(/^\/|\/$/g, '');
...
function populateMovePicker() {
  ...
  root.addEventListener('click', () => {
    picker.querySelectorAll('.picker-folder').forEach(el => el.classList.remove('selected'));
...
function renderLightboxSlide() {
  ...
  $('lightbox-next').style.visibility = state.lightboxIndex < mf.length - 1 ? 'visible' : 'hidden';
}

function navigateLightbox(dir) {
  const mf = mediaFiles();
  state.lightboxIndex = Math.max(0, Math.min(mf.length - 1, state.lightboxIndex + dir));
  renderLightboxSlide();
}
